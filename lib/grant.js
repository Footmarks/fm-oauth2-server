...
function extractCredentials(done) {
    ...
    if (this.req.body.grant_type === 'app_credentials') {
...
function checkCredentials(done) {
    ...
    if (this.client) {
        ...
        this.model.getApp({ appKey: this.app.appKey, appSecret: this.app.appSecret },
            function (err, app) {
                if (err) return done(error('server_error', false, err));
                ...
                self.app_more = new App_more(app._id, app.client);
                done();
            });
    }
...
function usePasswordGrant(done) {
    ...
    var self = this;
    return this.model.getAdminUser(uname, pword, function (err, user) {
        if (err) return done(error('server_error', false, err));
        if (!user) {
...
/**
 * Grant for app_credentials grant type
 *
 * @param  {Function} done
 */
function useAppCredentialsGrant(done) {
    ...
    var self = this;
    return this.model.getUserRegisteredToDevice({ deviceId: device, appId: id, clientId: client },
        function (err, user) {
            if (err) return done(error('server_error', false, err));
            if (!user) {
...
function checkGrantTypeAllowed(done) {
    var query = { grantType: this.grantType };
    if (this.client) query.clientId = this.client.clientId;
    else query.clientId = this.app_more.clientId;

    this.model.grantTypeAllowed(query,
...
function saveAccessToken(done) {
    ...
    if (this.client) {
        this.model.saveAccessToken({ accessToken: accessToken, company_Id: this.client.clientId, expires: expires, adminUser: this.user, grantType: this.grantType },
            function (err) {
        ...
        this.model.saveAccessToken({ accessToken: accessToken, company_Id: this.app_more.clientId, expires: expires, user: this.user, grantType: this.grantType },
            function (err) {
                if (err) return done(error('server_error', false, err));
                done();
            });
