...
function extractCredentials (done) {
  ...
  if (req.body.grant_type === 'app_credentials') {
...
function checkGrantTypeAllowed (done) {
    this.model.grantTypeAllowed({ clientId: this.client ? this.client.clientId : this.app.clientId, grantType: this.grantType },
...
function saveAccessToken (done) {
  ...
  if (this.client) {
      ...
      this.model.saveAccessToken({ accessToken: accessToken, clientId: this.app.clientId, expires: expires, appUser: this.user },
          function (err) {
            if (err) return done(error('server_error', false, err));
            done();
        });
