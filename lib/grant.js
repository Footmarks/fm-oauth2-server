    ...
    if (this.client) {
        ...
        this.model.getApp({ appKey: this.app.appKey, appSecret: this.app.appSecret },
            function (err, app) {
                ...
                console.log('after app_more created');
                done();
            });
    }
...
function saveAccessToken (done) {
  ...
  if (this.client) {
      this.model.saveAccessToken({ accessToken: accessToken, clientId: this.client.clientId, expires: expires, clientUser: this.user }, 
          function (err) {
            if (err) return done(error('server_error', false, err));
            done();
        });
  } else if (this.app) {
      this.model.saveAccessToken({ accessToken: accessToken, clientId: this.app_more.clientId, expires: expires, appUser: this.user },
          function (err) {
            if (err) return done(error('server_error', false, err));
            done();
        });
